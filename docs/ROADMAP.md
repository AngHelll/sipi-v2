...
4. **Backend (no bloqueante)**: revisar coherencia de `assign-group` con la regla canónica de disponibilidad. Hoy `assign-group` (admin, lista de espera) exige la **misma** regla del alumno (incl. ventana de inscripción vigente), lo que puede bloquear asignaciones admin legítimas fuera de fechas — a diferencia de `assign-period`, que sí permite al admin operar fuera de la ventana. Definir y alinear (probablemente: validar tipo/nivel + no eliminado + no `CERRADO`/`CANCELADO`/`FINALIZADO` + cupo, **sin** exigir la ventana pública).
...
- **Maestro**: vista de grupo de **solo lectura** (roster + horario); sección "Pendientes por calificar" con **urgencia por `fechaFin`** (≤7 días / hoy / finalizado) y orden por urgencia; **costo oculto** (dato administrativo).
...
- **Backend hardening**: `PUT /api/groups/:id` acepta cualquier campo (antes exigía nombre/periodo/subjectId/teacherId → 400 al cerrar); código de grupo `GRP-NNNNNN` desde el mayor sufijo (no `count()`, evita colisión al duplicar); invalidación de caché del listado en crear/editar/cerrar/eliminar/restaurar.
...
**Última actualización**: 2026-06-24 (sesión 2) — pasada por rol (maestro/alumno/admin) y ciclo de vida de cursos de inglés (duplicar/cerrar/baja lógica+restaurar+historial, materia canónica por nivel, costo oculto al maestro, urgencia de calificación), con hardening de backend de grupos (PUT de cualquier campo, código único, invalidación de caché) y feedback al contrato móvil. Previo: consolidación de la capa web admin, fix del filtro de exámenes (400) + hardening de tipos/tests, y auditoría de navegación/roles.
