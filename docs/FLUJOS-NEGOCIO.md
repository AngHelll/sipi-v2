...
- **Eliminar = baja lógica**: `DELETE /api/groups/:id` marca `deletedAt` (no borra físicamente ni sus inscripciones). Los grupos dados de baja salen de los listados activos y quedan en el **historial** (`GET /api/groups?eliminados=true`); se pueden **restaurar** con `POST /api/groups/:id/restore`. El listado de grupos está cacheado, por lo que crear/editar/cerrar/eliminar **invalida** la caché para reflejarse de inmediato.
- **Código de grupo (`GRP-NNNNNN`)**: se genera a partir del mayor sufijo existente (no de `count()`), garantizando unicidad aun con bajas/duplicados.
